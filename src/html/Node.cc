...
void Node::parseAttributes() 
{
	...
	while (is_ascii_space(*ptr)) ++ptr;

	// Skip tagname
	if (!isalpha((unsigned char)*ptr)) return;
	while (!is_ascii_space(*ptr) && *ptr != '>') ++ptr;
	...
	while (*ptr && *ptr != '>') 
	{
		...
		// skip unrecognized
		while (*ptr && !isalnum((unsigned char)*ptr) && !is_ascii_space(*ptr)) ++ptr;
...
//				fprintf(stderr, "Trying to find quote: %c\n", quote);
				...
				while (is_ascii_space(*begin) && begin < end) ++begin;
				const char *trimmed_end = end - 1;
				while (is_ascii_space(*trimmed_end) && trimmed_end >= begin) --trimmed_end;
			...
			else 
			{
				...
				while (*end && !is_ascii_space(*end) && *end != '>') end++;
				val.assign(ptr, end);
				ptr = end;
			}
