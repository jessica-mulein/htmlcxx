	...
	namespace HTML {
		...
		string strip_comments(const string &str) {
			...
			while(1) {
				if(!inside_comment) {
					if(ptr  + 4 < end) {
						if(*ptr == '<' && *(ptr+1) == '!' && *(ptr+2) =='-' && *(ptr + 3) == '-' && is_ascii_space(*(ptr + 4))) {
							inside_comment = true;
						}
					}
				} else {
...
//			printf("url_end: %s\n", ret.c_str());
		...
		string get_attribute(const string& tag, const string& attr) {
			...
			a += attr.length();
			while (a < tag.length() && is_ascii_space(tag[a])) a++;
			...
			if (tag[a] == '"') {
				...
				while (a < tag.length() && !is_ascii_space(tag[a]) && tag[a] != '>') {
					val += tag[a++];
				}
			}
