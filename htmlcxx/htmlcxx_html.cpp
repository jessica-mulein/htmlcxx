...
void Node::parseAttributes()
{
	...
	while (*ptr && *ptr != '>')
	{
		...
		else
		{
			if (!key.empty())
			{
				mAttributes.insert(make_pair(key, std::string()));
			}
		}
	}
}
...
void ParserDom::foundTag(Node node, bool isEnd)
{
	if (!isEnd)
	{
		...
		kp::tree<HTML::Node>::iterator next_state;
	...
	else
	{
		...
		// matches
		std::vector<kp::tree<HTML::Node>::iterator> path;
		kp::tree<HTML::Node>::iterator i = mCurrentState;
...
{
	kp::tree<Node>::pre_order_iterator it = tr.begin();
	kp::tree<Node>::pre_order_iterator end = tr.end();
